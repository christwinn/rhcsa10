if grep -v rhel /etc/chrony.conf | grep -v '^#' | grep 'pool[[:space:]]pool.ntp.org' &>/dev/null && \
   ssh server2 grep -v rhel /etc/chrony.conf | grep -v '^#' | grep 'pool[[:space:]]pool.ntp.org' &>/dev/null
then
